...
# Path Logic: Finds the 'frontend' folder one level up from the 'backend' folder
...
class InterviewSession(db.Model):
    ...
    def to_dict(self): return {'id': self.id, 'type': self.type, 'language': self.language, 'score': self.score, 'date': self.date, 'results': json.loads(self.details_json) if self.details_json else []}
...
class Job(db.Model):
    ...
    skills = db.Column(db.Text)
    def to_dict(self): return {'id': self.id, 'title': self.title, 'dept': self.department, 'loc': self.location, 'skills': self.skills}
...
def register():
    ...
    if User.query.filter_by(email=data['email']).first(): return jsonify({"error": "User exists"}), 409
...
def handle_profile():
    ...
    if request.method == 'POST':
        ...
        profile.role_title, profile.education = data.get('role'), data.get('education')
        profile.skills, profile.projects = data.get('skills'), data.get('projects')
        db.session.commit()
        return jsonify({"msg": "Updated"}), 200
    return jsonify(profile.to_dict() if profile else {})

# =========================================
# 4. RESUME & INTERVIEW ENDPOINTS
# =========================================
...
def parse_and_score(filepath):
    ...
    try:
        ...
        score = 40 + len([k for k in keywords if k in text.lower()]) * 6
        return min(score, 98)
    except: return 50
...
# =========================================
# 5. AI CODE ANALYSIS & HR ENDPOINTS
# =========================================
...
def analyze_code():
    ...
    try:
        res = requests.post(HF_MODEL_URL, headers={"Authorization": f"Bearer {HF_API_TOKEN}"}, json={"inputs": prompt}, timeout=10)
        raw = res.json()[0]['generated_text']
        ai_data = json.loads(raw[raw.find('{'):raw.rfind('}')+1])
        return jsonify(ai_data)
    except:
        return jsonify({"score": 75, "verdict": "Good", "feedback": "Code is functional. AI analysis fallback active."})
...
def hr_candidates():
    ...
    for c in candidates:
        ...
        res.append({
            "id": c.id, "name": c.name, "role": c.profile.role_title if c.profile else "N/A",
            "ats": r.ats_score if r else 0, "tech": i.score if i else 0, "status": "Reviewed" if i else "Pending"
        })
    return jsonify(res)
...
def manage_jobs():
    if request.method == 'POST':
        data = request.get_json()
        db.session.add(Job(title=data['title'], description=data.get('description', ''), department=data.get('department', ''), location=data.get('location', ''), skills=data.get('skills', '')))
        db.session.commit(); return jsonify({"msg": "Posted"}), 201
    return jsonify([j.to_dict() for j in Job.query.all()])

# =========================================
# 6. STATIC FILE SERVING (CATCH-ALL)
# =========================================
